...
def main(hparams, run=None, gpu_num=0):
    ...
    if policy == 'reinforce':
        ...
        model = ACTeacherStudentModel(_model_configs, noteach=True)
    else:
        raise RuntimeError('Invalid policy: {}. Please choose from [reinforce, actor_critic, noteach]'.format(policy))
    model.eval()
    model.cuda()
    ...
    print ('Testing teaching policy: {}'.format(policy))
    if policy != 'noteach':
        teacher_checkpoint_filename = model.load_teacher_checkpoint('')
        print('Loading trained teacher from checkpoint: {}'.format(teacher_checkpoint_filename))
